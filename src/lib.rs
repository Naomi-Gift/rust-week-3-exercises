...
impl CompactSize {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        match self.value {
            ...
            // [0xFDxxxx] => 0xFD + u16 (2 bytes, little-endian)
            0xFD..=0xFFFF => {
                let mut bytes = vec![0xFD];
                bytes.extend_from_slice(&(self.value as u16).to_le_bytes());
                bytes
            }
            // [0xFExxxxxxxx] => 0xFE + u32 (4 bytes, little-endian)
            0x10000..=0xFFFFFFFF => {
                let mut bytes = vec![0xFE];
                bytes.extend_from_slice(&(self.value as u32).to_le_bytes());
                bytes
            }
            // [0xFFxxxxxxxxxxxxxxxx] => 0xFF + u64 (8 bytes, little-endian)
            _ => {
...
impl BitcoinTransaction {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (input_count, cs_len) = CompactSize::from_bytes(&bytes[offset..])?;
        offset += cs_len;
